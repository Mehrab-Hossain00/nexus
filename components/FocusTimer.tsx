...
import { Play, Pause, RotateCcw, Brain, Plus, X, Volume2, VolumeX, Settings2, Check, FastForward, Maximize2, Minimize2 } from 'lucide-react';
    ...
    return (
      <div className="fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center animate-fade-in overflow-hidden">
        ...
        <button 
          ...
          className="absolute top-10 right-10 p-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-3xl text-zinc-600 hover:text-white transition-all backdrop-blur-3xl group"
        ...
        <div className="relative flex items-center justify-center select-none" style={{ width: size, height: size }}>
            ...
            <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-full transform -rotate-90 absolute inset-0">
                ...
                <circle cx={size/2} cy={size/2} r={radius} strokeWidth="3" stroke="currentColor" fill="transparent" 
                  ...
                  strokeLinecap="round"
                  className={`${globalTimer.mode === 'focus' ? 'text-nexus-electric' : 'text-emerald-500'} transition-all duration-700 ease-out`}
            ...
            <div className="flex flex-col items-center justify-center text-center z-10">
                ...
                <div className="mt-6 flex flex-col items-center gap-4 animate-fade-in">
                  <div className={`px-8 py-2 rounded-full border text-[10px] font-black uppercase tracking-[0.5em] ${globalTimer.mode === 'focus' ? 'bg-nexus-electric/10 border-nexus-electric/20 text-nexus-electric' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500'}`}>
                    {globalTimer.mode === 'focus' ? 'High Focus' : 'System Recovery'}
                  </div>
        ...
        {/* Dynamic Controls Bar - Fades out for zero distraction */}
        <div className="absolute bottom-20 flex items-center gap-12 transition-all duration-700 opacity-20 hover:opacity-100 transform translate-y-4 hover:translate-y-0">
            <button onClick={handleReset} className="w-20 h-20 rounded-full bg-zinc-900/50 border border-white/5 text-zinc-600 flex items-center justify-center hover:text-white hover:border-white/20 transition-all active:scale-90">
                <RotateCcw className="w-6 h-6" />
            </button>
            <button onClick={toggleTimer} className="w-32 h-32 rounded-full bg-white text-black flex items-center justify-center shadow-[0_0_60px_rgba(255,255,255,0.15)] active:scale-90 transition-all hover:scale-105">
                {globalTimer.isActive ? <Pause className="w-12 h-12 fill-black" /> : <Play className="w-12 h-12 fill-black ml-2" />}
            </button>
            <button 
              onClick={() => globalTimer.manualEnd()} 
              className="w-20 h-20 rounded-full bg-nexus-electric/10 border border-nexus-electric/20 text-nexus-electric flex items-center justify-center hover:bg-nexus-electric hover:text-white transition-all active:scale-90"
            >
    ...
    <div className="h-full overflow-y-auto custom-scrollbar pr-2 pb-12 animate-fade-in relative">
      <div className="absolute top-4 right-4 z-20 flex flex-col gap-3">
          <button 
            ...
            className="p-3 bg-zinc-900/60 border border-white/10 rounded-2xl text-zinc-500 hover:text-white transition-all backdrop-blur-xl shadow-xl hover:scale-105 active:scale-95"
