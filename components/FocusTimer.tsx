...
export const FocusTimer: React.FC<FocusTimerProps> = ({ user, subjects, setSubjects, onTriggerXP, onUpdateQuest, globalTimer }) => {
  ...
  const renderFullscreen = () => (
    <div className="fixed inset-0 z-[9999] bg-nexus-black flex flex-col items-center justify-center animate-fade-in overflow-hidden">
      ...
      <button onClick={() => setIsFullscreen(false)} className="absolute top-10 right-10 p-4 bg-white/5 hover:bg-white/10 border border-nexus-border rounded-3xl text-zinc-600 hover:text-white transition-all backdrop-blur-3xl group" title="Exit (ESC)">
      ...
      <div className="absolute bottom-20 flex items-center gap-12 transition-all duration-700 opacity-20 hover:opacity-100 transform translate-y-4 hover:translate-y-0">
          <button onClick={handleReset} className="w-20 h-20 rounded-full bg-nexus-card border border-nexus-border text-zinc-600 flex items-center justify-center hover:text-white hover:border-white/20 transition-all active:scale-90"><RotateCcw className="w-6 h-6" /></button>
  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar pr-2 pb-12 animate-fade-in relative">
      <div className="absolute top-4 right-4 z-20 flex flex-col gap-3">
          <button onClick={() => setIsFullscreen(true)} className="p-3 bg-nexus-card/50 border border-nexus-border rounded-2xl text-zinc-400 hover:text-white hover:bg-nexus-card transition-all backdrop-blur-md shadow-sm hover:scale-105 active:scale-95" title="Fullscreen Timer"><Maximize2 className="w-5 h-5" /></button>
          <button onClick={() => globalTimer.setIsMuted(!globalTimer.isMuted)} className={`p-3 border border-nexus-border rounded-2xl transition-all backdrop-blur-md shadow-sm hover:scale-105 active:scale-95 ${globalTimer.isMuted ? 'bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 border-rose-500/20' : 'bg-nexus-card/50 text-zinc-400 hover:text-white hover:bg-nexus-card'}`} title={globalTimer.isMuted ? "Unmute sounds" : "Mute sounds"}>{globalTimer.isMuted ? <VolumeX className="w-5 h-5" /> : <Volume2 className="w-5 h-5" />}</button>
          <button onClick={() => { setEditSettings(settings); setShowConfig(true); }} className="p-3 bg-nexus-card/50 border border-nexus-border rounded-2xl text-zinc-400 hover:text-white hover:bg-nexus-card transition-all backdrop-blur-md shadow-sm hover:scale-105 active:scale-95" title="Timer Settings"><Settings2 className="w-5 h-5" /></button>
      </div>

      <div className="min-h-full flex flex-col items-center justify-center space-y-12 py-8 relative">
        <div className="flex flex-col items-center gap-6">
          <div className="flex p-1.5 bg-nexus-card/40 border border-nexus-border rounded-full backdrop-blur-md shrink-0">
              <button onClick={() => switchType('stopwatch')} className={`px-8 py-2.5 rounded-full text-xs font-bold transition-all flex items-center gap-2 ${globalTimer.type === 'stopwatch' ? 'bg-white text-black shadow-md' : 'text-zinc-500 hover:text-zinc-300'}`}><TimerIcon className="w-4 h-4" /> STOPWATCH</button>
              <button onClick={() => switchType('pomodoro')} className={`px-8 py-2.5 rounded-full text-xs font-bold transition-all flex items-center gap-2 ${globalTimer.type === 'pomodoro' ? 'bg-white text-black shadow-md' : 'text-zinc-500 hover:text-zinc-300'}`}><Hourglass className="w-4 h-4" /> POMODORO</button>
          </div>
          {globalTimer.type === 'pomodoro' && (
            <div className="flex p-1 bg-nexus-card/30 border border-nexus-border rounded-full backdrop-blur-sm shrink-0 animate-fade-in">
                {(['focus', 'short', 'long'] as const).map(m => (
                    <button key={m} onClick={() => switchMode(m)} className={`px-6 py-1.5 rounded-full text-[10px] font-black tracking-widest transition-all ${globalTimer.mode === m ? 'bg-white/10 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>{m.toUpperCase()}</button>
                ))}
            </div>
          )}
        </div>
        ...
        {globalTimer.mode === 'focus' && !globalTimer.isActive && (
            <div className="w-full max-w-xl space-y-4 animate-fade-in px-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {subjects.map(s => (
                        <div key={s} onClick={() => globalTimer.setSubject(s)} className={`py-3 rounded-2xl text-xs font-medium border transition-all flex items-center justify-center relative group cursor-pointer ${globalTimer.subject === s ? 'bg-nexus-electric/10 border-nexus-electric text-nexus-electric shadow-[0_0_15px_rgba(var(--nexus-accent-rgb),0.1)]' : 'bg-nexus-card/40 border-nexus-border text-zinc-400 hover:bg-nexus-card hover:text-zinc-200'}`}>
                            <span className="truncate px-3">{s}</span>
                            {subjects.length > 1 && <button onClick={(e) => deleteSubject(e, s)} className="absolute -top-2 -right-2 p-1.5 bg-nexus-card border border-nexus-border rounded-full text-zinc-500 hover:text-rose-400 hover:bg-rose-500/10 hover:border-rose-500/30 opacity-0 group-hover:opacity-100 transition-all shadow-sm"><X className="w-3 h-3" /></button>}
                        </div>
                    ))}
                    {isAddingSubject ? (
                      <form onSubmit={addSubject} className="flex"><input autoFocus value={newSubjectName} onChange={(e) => setNewSubjectName(e.target.value)} onBlur={() => { if (!newSubjectName.trim()) setIsAddingSubject(false); else addSubject(); }} placeholder="Name..." className="w-full py-3 bg-nexus-card border border-nexus-electric/50 rounded-2xl text-xs text-white px-4 outline-none shadow-[0_0_10px_rgba(var(--nexus-accent-rgb),0.1)]" /></form>
                    ) : (
                      <button onClick={() => setIsAddingSubject(true)} className="py-3 rounded-2xl text-xs font-medium border border-dashed border-zinc-700 text-zinc-500 hover:text-zinc-300 hover:border-zinc-500 hover:bg-nexus-card/30 transition-all flex items-center justify-center gap-2"><Plus className="w-4 h-4" /><span>Add</span></button>
                    )}
                </div>
            </div>
        )}
        ...
        {globalTimer.isActive && globalTimer.mode === 'focus' && (
            <div className="px-6 py-2.5 bg-nexus-electric/10 border border-nexus-electric/20 rounded-full text-nexus-electric text-xs font-bold animate-pulse flex items-center gap-2 shadow-[0_0_15px_rgba(var(--nexus-accent-rgb),0.15)]"><Brain className="w-4 h-4" />Studying {globalTimer.subject}</div>
        )}
        ...
        <div className="relative group flex items-center justify-center shrink-0" style={{ width: 340, height: 340 }}>
            <svg viewBox={`0 0 340 340`} className="w-full h-full transform -rotate-90">
                <circle cx={170} cy={170} r={140} strokeWidth="6" stroke="currentColor" fill="transparent" className="text-nexus-border opacity-50" />
                <circle cx={170} cy={170} r={140} strokeWidth="6" stroke="currentColor" fill="transparent" 
                  strokeDasharray={2 * Math.PI * 140}
                  strokeDashoffset={isPomodoro ? (2 * Math.PI * 140 * (1 - progress / 100)) : (globalTimer.isActive ? undefined : 2 * Math.PI * 140)}
                  strokeLinecap="round"
                  className={`${globalTimer.mode === 'focus' ? 'text-nexus-electric' : 'text-emerald-500'} transition-all duration-500 ease-out ${!isPomodoro && globalTimer.isActive ? 'animate-pulse' : ''}`}
                  style={{ filter: `drop-shadow(0 0 12px ${globalTimer.mode === 'focus' ? 'rgba(var(--nexus-accent-rgb),0.4)' : 'rgba(16,185,129,0.4)'})` }}
                />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <div className="mt-6 px-5 py-2 bg-nexus-card/80 backdrop-blur-md border border-nexus-border rounded-full text-[10px] font-bold text-zinc-400 uppercase tracking-[0.25em] shadow-sm">{globalTimer.type} • {globalTimer.mode}</div>
            </div>
        </div>

        <div className="flex items-center gap-8 shrink-0">
            <button onClick={handleReset} className="w-16 h-16 rounded-full bg-nexus-card/50 border border-nexus-border text-zinc-400 flex items-center justify-center hover:text-white hover:bg-nexus-card transition-all active:scale-90 backdrop-blur-md shadow-sm" title="Reset Session"><RotateCcw className="w-5 h-5" /></button>
            <button onClick={toggleTimer} className="w-24 h-24 rounded-full bg-white text-black flex items-center justify-center shadow-[0_0_40px_rgba(255,255,255,0.15)] active:scale-90 transition-all hover:scale-105">{globalTimer.isActive ? <Pause className="w-10 h-10 fill-black" /> : <Play className="w-10 h-10 fill-black ml-1.5" />}</button>
            {(globalTimer.isActive || (!isPomodoro && globalTimer.timeValue > 0)) ? (
                <button onClick={() => globalTimer.manualEnd()} className="w-16 h-16 rounded-full bg-nexus-electric/10 border border-nexus-electric/20 text-nexus-electric flex items-center justify-center hover:bg-nexus-electric hover:text-white transition-all active:scale-90 group shadow-sm" title="Finish and Save Progress"><FastForward className="w-5 h-5 group-hover:translate-x-0.5 transition-transform" /></button>
            ) : (
                <button onClick={() => setShowManual(true)} className="w-16 h-16 rounded-full bg-nexus-card/50 border border-nexus-border text-zinc-400 flex items-center justify-center hover:text-white hover:bg-nexus-card transition-all active:scale-90 backdrop-blur-md shadow-sm" title="Add Manual Session"><Plus className="w-5 h-5" /></button>
            )}
        </div>
      </div>
      ...
      {showManual && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-nexus-black/80 backdrop-blur-md animate-fade-in">
          <div className="w-full max-w-sm bg-[#09090b] border border-nexus-border rounded-3xl p-8 shadow-2xl relative overflow-hidden">
                ...
                <div className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <select 
                          ...
                          className="w-full bg-nexus-card border border-nexus-border rounded-xl px-4 py-3 text-sm text-white focus:border-nexus-electric outline-none appearance-none"
                        >
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-3">
                            <input 
                              ...
                              className="w-full bg-nexus-card border border-nexus-border rounded-xl px-4 py-3 text-white focus:border-nexus-electric outline-none font-mono" 
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    {(['focus', 'short', 'long'] as const).map(m => (
                      <div key={m} className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-3">
                              <input type="number" value={editSettings[m]} onChange={e => setEditSettings({...editSettings, [m]: parseInt(e.target.value) || 1})} className="w-16 bg-nexus-card border border-nexus-border rounded-lg px-2 py-1.5 text-center text-white focus:border-nexus-electric outline-none font-mono" />
