...
const TASK_COLORS = ['#6366f1', '#27272a'];
...
export const Analytics: React.FC<AnalyticsProps> = ({ user }) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
        ...
        avgSession: sessions.length > 0 ? Math.round((totalSecs / sessions.length) / 60) : 0,
        nodesCount: sessions.length,
  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar space-y-8 animate-fade-in pb-20 pr-2">
         ...
         <div className="flex bg-zinc-900/40 p-1 rounded-xl border border-white/5 backdrop-blur-md">
            {(['Daily', 'Weekly', 'Monthly', 'Yearly'] as TimeRange[]).map(r => (
                <button
                    ...
                    className={`px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all ${timeRange === r ? 'bg-indigo-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                    {r}
                </button>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={Clock} value={`${stats.totalHours}h`} label="Total Study Time" color="text-indigo-400" />
          <StatCard icon={Activity} value={`${stats.avgSession}m`} label="Average Session" color="text-purple-400" />
                    ...
                    return (
                        <div key={item.name} className="space-y-3 group">
                            <div className="flex justify-between items-end">
                                <span className="text-xs font-bold text-white group-hover:text-indigo-400 transition-colors truncate pr-2">{item.name}</span>
